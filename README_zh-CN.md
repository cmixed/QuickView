...
> **为何要多带一个参数？** 预设中故意把 `VCPKG_MANIFEST_INSTALL` 固定为 `OFF`，以保证重复配置时保持极快速度、且不会卡在 vcpkg 安装环节。因此依赖需要在**首次**配置时显式拉取，`-DVCPKG_MANIFEST_INSTALL=ON` 就是做这件事；之后直接使用 `cmake --preset Release-LTO` 即可。每当 `vcpkg.json` 发生变化时，请重新加上该参数。

首次配置会从源码编译全部依赖（libjxl、libavif、LibRaw 等），耗时较长；之后的配置会复用 vcpkg 二进制缓存。所有依赖都会被安装到 `out/build/Release-LTO/vcpkg_installed`。
